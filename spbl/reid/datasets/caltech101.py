...
class caltech101(Dataset):
    ...
    def __init__(self, root, split_id=0, num_val=100, download=True):
        super(caltech101, self).__init__(root, split_id=split_id)
    ...
    def download(self):
        ...
        identities = [[[] for _ in range(1)] for _ in range(102)]
        identities_train = [[[] for _ in range(1)] for _ in range(102)]
        label_train = [[[] for _ in range(1)] for _ in range(102)]
        identities_test = [[[] for _ in range(1)] for _ in range(102)]
        label_test = [[[] for _ in range(1)] for _ in range(102)]

        def register_w(subdir):
            pids = set()
            file = open(osp.join('/home/ubuntu1/wxp/TNNLS2018/open-reid-master_sd198/examples/data/caltech101/raw', subdir), 'r')
            while 1:
                line = file.readline()
                if not line:
                    break
