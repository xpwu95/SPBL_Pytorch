...
class CIFAR100(Dataset):
    ...
    def __init__(self, root, split_id=0, num_val=100, download=True):
        super(CIFAR100, self).__init__(root, split_id=split_id)
    ...
    def download(self):
        ...
        identities = [[[] for _ in range(1)] for _ in range(100)]
        identities_train = [[[] for _ in range(1)] for _ in range(100)]
        label_train = [[[] for _ in range(1)] for _ in range(100)]
        identities_test = [[[] for _ in range(1)] for _ in range(100)]
        label_test = [[[] for _ in range(1)] for _ in range(100)]

        def register_w(subdir):
            pids = set()
            file = open(osp.join('/home/ubuntu/wxp/TNNLS2018/open-reid-master_sd198_lite/examples/data/cifar-100/raw', subdir), 'r')
            while 1:
                line = file.readline()
                if not line:
                    break
