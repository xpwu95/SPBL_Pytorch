...
class ACNE_LEVEL4(Dataset):
    #url = 'cv.nankai.edu.cn/projects/sd-198/SD-198.zip'
    #md5 = ''

    def __init__(self, root, split_id=0, num_val=100, download=True):
        super(ACNE_LEVEL4, self).__init__(root, split_id=split_id)
    ...
    def download(self):
        ...
        identities = [[[] for _ in range(1)] for _ in range(4)]
        identities_train = [[[] for _ in range(1)] for _ in range(4)]
        label_train = [[[] for _ in range(1)] for _ in range(4)]
        identities_test = [[[] for _ in range(1)] for _ in range(4)]
        label_test = [[[] for _ in range(1)] for _ in range(4)]

        def register_w(subdir):
            pids = set()
            file = open(osp.join('/home/ubuntu3/wxp/TNNLS2018/open-reid-master_sd198/examples/data/acne_level4/raw', subdir), 'r')
            while 1:
                line = file.readline()
                if not line:
                    break
        ...
        meta = {'name': 'acne_level4', 'shot': 'multiple', 'num_cameras': 6,
