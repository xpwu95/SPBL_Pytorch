...
class MNIST(Dataset):
    ...
    def __init__(self, root, split_id=0, num_val=100, download=True):
        super(MNIST, self).__init__(root, split_id=split_id)
    ...
    def download(self):
        ...
        identities_train = [[[] for _ in range(1)] for _ in range(10)]
        label_train = [[[] for _ in range(1)] for _ in range(10)]
        identities_test = [[[] for _ in range(1)] for _ in range(10)]
        label_test = [[[] for _ in range(1)] for _ in range(10)]

        def register_w(subdir):
            pids = set()
            file = open(osp.join('/home/ubuntu1/wxp/TNNLS2018/open-reid-master_sd198/examples/data/mnist/raw', subdir), 'r')
            while 1:
                line = file.readline()
                if not line:
                    break
        ...
        trainval_pids = register_w('train_4.txt')
        #gallery_pids = register('bounding_box_test')
        gallery_pids = register_w('val_4.txt')
