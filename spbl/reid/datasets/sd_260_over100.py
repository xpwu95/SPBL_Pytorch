...
class SD_260_OVER100(Dataset):
    #url = 'cv.nankai.edu.cn/projects/sd-198/SD-198.zip'
    #md5 = ''

    def __init__(self, root, split_id=0, num_val=100, download=True):
        super(SD_260_OVER100, self).__init__(root, split_id=split_id)
    ...
    def download(self):
        ...
        identities = [[[] for _ in range(1)] for _ in range(39)]
        identities_train = [[[] for _ in range(1)] for _ in range(39)]
        label_train = [[[] for _ in range(1)] for _ in range(39)]
        identities_test = [[[] for _ in range(1)] for _ in range(39)]
        label_test = [[[] for _ in range(1)] for _ in range(39)]

        def register_w(subdir):
            pids = set()
            file = open(osp.join('/home/ubuntu3/wxp/TNNLS2018/open-reid-master_sd198/examples/data/sd-260-over100/raw', subdir), 'r')
            while 1:
                line = file.readline()
                if not line:
                    break
        ...
        meta = {'name': 'sd_260_over100', 'shot': 'multiple', 'num_cameras': 6,
